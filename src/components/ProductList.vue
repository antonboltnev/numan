      ...
      >
        ...
        >
          ...
          <div class="ProductList__item">
            ...
            <div
              class="item__availability"
              v-if="product.attributes.availability"
            >
              <p>Availability:</p>
...
export default {
  ...
  methods: {
    ...
    getProductList() {
      ...
      //getting products from 'included' by product ID
      localProducts.map((prod) => {
        ...
        //setting a default selected product
        this.localProducts.map((product) => {
          this.$set(product, "default", false);
          this.products.data.map((item) => {
            if (item.relationships.default_product.data.id === product.id)
    ...
    getProductVariants(product) {
      let exactProduct = this.localProducts[
        this.localProducts.indexOf(product)
      ];
      ...
      this.products.included.map((item) => {
        if (
          ...
          let localItem = {
            default:
              product.relationships.default_product_variant.data.id === item.id,
            ...item,
          };
    ...
    selectVariant(product, variant) {
      this.localProducts[this.localProducts.indexOf(product)].variants.map(
        (item) => {
...
.ProductList {
  ...
  .item {
    &__description p:first-of-type,
    &__availability p:first-of-type,
    &__variants p:first-of-type {
      font-weight: bolder;
    }
  }
  .product_variant {
